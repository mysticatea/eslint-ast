import { AliasName, ASTTypeOf, Definition, NodeName, NodeNameOfAlias } from "./definition";
...
declare type NodePropertyNames<D extends Definition, N extends NodeName<D>> = "loc" | "parent" | "range" | "type" | keyof D["nodes"][N];
/**
...
declare type ResolveNodeRef_<D extends Definition, V> = V extends {
    $ref: infer N;
} ? N extends "Node" ? NodeOfNodeName<D, NodeName<D>> : N extends AliasName<D> ? NodeOfNodeName<D, NodeNameOfAlias<D, N>> : N extends NodeName<D> ? Node<D, N> : {
    "!! UNKNOWN NODE NAME !!": N;
...
declare type NameOfNodeRef_<D extends Definition, V> = V extends {
    $ref: infer N;
} ? N extends "Node" ? NodeName<D> : N extends AliasName<D> ? NodeNameOfAlias<D, N> : N extends NodeName<D> ? N : never : never;
/**
 * Resolve given `NodeRef<T>` or `NodeRef<T>[]` to the corresponded nodes.
 * @template D The AST definition.
 * @template V The value that may be `NodeRef<T>`
 */
declare type ResolveNodeRef<D extends Definition, V> = V extends readonly (infer E)[] ? readonly ResolveNodeRef_<D, E>[] : ResolveNodeRef_<D, V>;
/**
 * Get the `T` of `NodeRef<T>` or `NodeRef<T>[]`.
 * @template D The AST definition.
 * @template V The value that may be `NodeRef<T>`
...
export declare type Node<D extends Definition, N extends NodeName<D>> = {
    readonly [P in NodePropertyNames<D, N>]: P extends "loc" ? LineColumnRange : P extends "parent" ? ParentNode<D, N> : P extends "range" ? IndexRange : P extends "type" ? ASTTypeOf<D, N> : P extends keyof D["nodes"][N] ? ResolveNodeRef<D, D["nodes"][N][P]> : never;
};
export {};
